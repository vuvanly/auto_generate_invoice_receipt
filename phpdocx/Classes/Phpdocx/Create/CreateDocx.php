...
class CreateDocx extends CreateDocument
{
    ...
    public function __construct($baseTemplatePath = PHPDOCX_BASE_TEMPLATE, $docxTemplatePath = '')
    {
        ...
        if (file_exists(dirname(__FILE__) . '/../Utilities/DOCXStructureTemplate.inc')) {
        ...
        if (isset($this->_phpdocxconfig['settings']['stream']) && 
            ...
            file_exists(dirname(__FILE__) . '/../Utilities/ZipStream.inc')) {
     ...
     *  Values: 'title', 'subject', 'creator', 'keywords', 'description', 'created' (W3CDTF without time zone), 'modified' (W3CDTF without time zone)
    ...
    public function addProperties($values)
    {
        ...
        if (!empty($values['title']) || !empty($values['subject']) || !empty($values['creator']) || !empty($values['keywords']) || !empty($values['description']) || !empty($values['category']) || !empty($values['contentStatus']) || !empty($values['created']) || !empty($values['modified'])) {
    ...
    {
        if (!file_exists(dirname(__FILE__) . '/../Crypto/CryptoPHPDOCX.inc')) {
    ...
    {
        if (!file_exists(dirname(__FILE__) . '/../Utilities/DOCXPath.inc')) {
            PhpdocxLogger::logger('This method is not available for your license.', 'fatal');
        }
     ...
     *     'type' (string) break, image, list, paragraph, run, section, style, table
    ...
    {
        if (!file_exists(dirname(__FILE__) . '/../Utilities/DOCXCustomizer.inc')) {
            PhpdocxLogger::logger('This method is not available for your license.', 'fatal');
        }
    ...
    /**
     * Regemerates a XML content based on its target after doing changes in it
     *
     * @access protected
